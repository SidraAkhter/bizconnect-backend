...
const taskAI_1 = require("../utils/taskAI");
exports.createTaskController = (0, asyncHandler_middleware_1.asyncHandler)(async (req, res) => {
    ...
    if (!assignedTo) {
        const bestUser = await (0, taskAI_1.suggestBestUser)();
        if (!bestUser) {
            return res.status(400).json({ message: "No user available to assign task" });
        }
        assignedTo = bestUser._id.toString();
    }
    const taskPayload = { ...body, assignedTo };
    const { task } = await (0, task_service_1.createTaskService)(workspaceId, projectId, userId, taskPayload);
    const assignedUser = await user_model_1.default.findById(task.assignedTo);
    if (assignedUser?.email) {
        await (0, mailer_1.sendEmail)(assignedUser.email, "📋 New Task Assigned", `You have been assigned a new task: ${task.title}`);
    }
